...
import { Image, Platform, StyleSheet, ToolbarAndroid, TouchableOpacity, View } from 'react-native';

import MBColors from './MBColors';
import {NormalText} from './MBText';
...
class MBHeaderAndroid extends Component {
    props: Props;
...
class MBHeaderIOS extends Component{
    props: Props;

    render(){
        const {leftItem, title, rightItem, foreground} = this.props;
        const titleColor = foreground === 'dark' ? MBColors.darkText : 'white';
        const itemsColor = foreground === 'dark' ? MBColors.lightText : 'white';
...
const Header = Platform.OS === 'ios' ? MBHeaderIOS : MBHeaderAndroid;
