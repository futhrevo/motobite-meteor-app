...
function _encode(coordinate, factor) {
...
function polyDecode(str, precision) {
  let index = 0,
    lat = 0,
    lng = 0,
    coordinates = [],
...
function polyEncode(coordinates, precision) {
  if (!coordinates.length) return '';

  let factor = Math.pow(10, precision || 5),
    output = _encode(coordinates[0][0], factor) + _encode(coordinates[0][1], factor);
...
function hashdecode(str, precision) {
  let index = 0,
    lat = 0,
    lng = 0,
    coordinates = [],
  ...
  while (index < str.length) {
    ...
    if ((lat != 0 && lng != 0) && (latitude_change > 100 || longitude_change > 100)) {
      ...
      const interp = (latfactor > lngfactor) ? latfactor : lngfactor;
      console.log(interp);

      for (let i = 0; i < interp; i++) {
        lat += latitude_change / interp;
        lng += longitude_change / interp;
                // coordinates.push([lat / factor, lng / factor]);
        coordinates.push([lng / factor, lat / factor]);
        var temp = encodeGeoHash(lat / factor, lng / factor, 6);
        if (dup != temp) {
          dup = temp;
          gh6.push(temp);
        }
...
function dissect(str, srcHash, dstHash, src, dest) {
  let index = 0,
    lat = 0,
    lng = 0,
    coordinates = [],
  ...
  while (index < str.length) {
    ...
    coordinates.push([lat / factor, lng / factor]);
    hashes.push(encodeGeoHash(lat / factor, lng / factor, 6));
  }


  const srcInfo = distanceToLine(src, coordinates);
  const destInfo = distanceToLine(dest, coordinates);
  const srcIndex = srcInfo.i;
  const destIndex = destInfo.i;
  const temp = coordinates.slice(srcIndex, destIndex);
    // add the start point and destination point to new array
  temp[0] = [srcInfo.x, srcInfo.y];
  temp.push([destInfo.x, destInfo.y]);

  return {
    overview: polyEncode(temp),
    srcDistance: srcInfo.distance,
    destDistance: destInfo.distance,
  };
}

module.exports = { polyDecode, polyEncode, hashdecode, dissect };
