...
function MeteorCreateUser(email: String, password: String): ThunkAction {
  return (dispatch) => {
    ...
    create.then(
      (result) => {
        console.log(result);
        dispatch(result);  // User object is not received when creating account, hence logging in forcefully
      }
    );
...
module.exports = { MeteorloginWithPassword, MeteorCreateUser, MeteorChangePassword, logOut, logOutWithPrompt, forgotPassword };
