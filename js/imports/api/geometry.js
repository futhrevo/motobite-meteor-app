...
import { polyDecode } from './PolylineHelpers';

const pole = 20037508.34;
// function to convert coordinates to cartesian system
function forwardMercator(xy) {
  xy.x = xy.x * pole / 180;
  const y = Math.log(Math.tan((90 + xy.y) * Math.PI / 360)) / Math.PI * pole;
...
function distanceToSegment(point, pointA, pointB) {
  ...
  const dx = x2 - x1;
  const dy = y2 - y1;
  const along = (dx == 0 && dy == 0) ? 0 : ((dx * (x0 - x1)) + (dy * (y0 - y1))) /
...
function segmentedLine(str) {
  const coordinates = polyDecode(str);
  const output = [];
...
function distanceToLine(point, str) {
  ...
  for (let i = 0; i < segment.length - 1; i++) {
    const result = distanceToSegment(point, segment[i], segment[i + 1]);
    if (min !== null) {
