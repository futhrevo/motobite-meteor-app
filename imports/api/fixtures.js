...
import { MarkerColl, ULogsColl, DriversAdvtColl, DrivesAdvtColl, TransactColl, SafeHouseColl } from './collections/collectionsInit';

Meteor.methods({
    // Entry for location
  postLocation(postAttributes) {
    if (!this.userId) {
      return;
    }
    ...
    const qw = MarkerColl.findOne({ id: userid });
    let post;
    if (!qw) {
  ...
  rideQuery(post) {
    ...
    // var result = DriversAdvtColl.find(
    // { "locs": { $near: { $geometry: { type: "Point", coordinates:post[0] },$maxDistance: 200}}},
    // { fields: { "locs":0 } });
    let ids = [];
    ...
    const nearSrc = DriversAdvtColl.aggregate([{
      $geoNear: {
        near: {
          type: 'Point',
    ...
    let nearDst = DriversAdvtColl.aggregate([{
      ...
      if (srcIndex < dstIndex) {
        for (let j = 0; j < nearSrc.length; j++) {
          if (u._id === nearSrc[j]._id) {
            const ret = {
              _id: u._id,
              srcDist: nearSrc[j].srcDist,
              dstDist: u.dstDist,
              srcIndex,
              dstIndex,
            };
    ...
    nearDst = nearDst.filter(element => element !== null);
    ...
    console.log(ids);
    const driverpool = DriversAdvtColl.find({
      _id: { $in: ids },
    }, {
      fields: {
        bounds: 1,
  ...
  riderQuery(post) {
    ...
    const drivepool = DrivesAdvtColl.find({ 'nodes.locs': { $geoWithin: { $box: [[77.676245, 12.926030], [100, 100]] } } });
    return drivepool.fetch();
  },
  postDriveReq(postAttributes) {
    if (!this.userId) {
    ...
    const insertId = DrivesAdvtColl.insert(postAttributes);
    return insertId;
  },
    // Entry for drive advertisement
  postDriveAdvt(postAttributes) {
    if (!this.userId) {
      return 'not a valid user';
    }
    check(this.userId, String);
...
//
  ...
  AskRider(obj) {
    ...
    const requestId = TransactColl.insert(post);
    DriversAdvtColl.update({ _id: obj._id },
        { $push: { pending: { requestId, requester } } }, { upsert: true });
  ...
  RiderActions(obj) {
    ...
    const advt = TransactColl.findOne({ _id: obj._id },
        { fields: { advtRequest: 1, requester: 1 } });
    console.log(advt);
        // check if user is in pending state and pull
    const state = DriversAdvtColl.update({ _id: advt.advtRequest, 'pending.requester': advt.requester }, { $pull: { pending: { requester: advt.requester } } });
    console.log(state);
    if (state > 0) {
      let send;
      if (obj.status) {
        DriversAdvtColl.update({ _id: advt.advtRequest },
            { $push: { accepted: { requestId: obj._id, requester: advt.requester } } },
            { upsert: true });
        send = { type: 'success', message: 'Request Accepted' };
      } else {
        DriversAdvtColl.update({ _id: advt.advtRequest },
            { $push: { rejected: { requestId: obj._id, requester: advt.requester } } },
            { upsert: true });
  ...
  deleteRide(obj) {
    ...
    const data = DriversAdvtColl.findOne({ _id: obj._id, id: this.userId },
        { fields: { pending: 1, accepted: 1, rejected: 1 } });
    let pendReq = [];
    let accReq = [];
    let rejReq = [];
    if (data.pending) {
