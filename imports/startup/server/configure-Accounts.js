...
Accounts.emailTemplates.resetPassword = {
  subject() {
    return 'Reset your password on MotoBite';
  },
  html(user, url) {
...
Accounts.onCreateUser((options, user) => {
  ...
  if (Object.prototype.hasOwnProperty.call(options.profile, 'workEmail')) {
    user.works.emails = [{ address: options.profile.workEmail, verified: false }];
    delete options.profile.workEmail;
  } else {
    user.works.emails = [];
  }
  if (Object.prototype.hasOwnProperty.call(options.profile, 'mobile')) {
...
Accounts.validateNewUser((user) => {
  ...
  if (user.mobile[0].number.length === 10) { return true; }
  throw new Meteor.Error(403, 'Mobile Number is not valid');
});
Accounts.validateNewUser((user) => {
  if (user.mobile.length === 0) {
    return true;
  }
  const dupMobile = Meteor.users.findOne({ 'mobile.number': user.mobile[0].number });
  if (!dupMobile) { return true; }
  throw new Meteor.Error(403, 'Mobile Number Already exist');
});
// TODO: Validate username, sending a specific error message on failure.
// Accounts.validateNewUser((user) => {
//   if (user.profile.name && user.profile.name.length >= 3) { return true; }
